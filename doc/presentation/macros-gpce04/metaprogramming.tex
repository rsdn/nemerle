...
with external code. Although we allow only one level of quotation (to avoid confusion
...
it isn't done entirely with quotations. We use special API, designed 
basing on \verb,System.Reflection,, which is used in \net\ for dynamic
generation of assemblies. As \nem\ type system and tasks we are performing
in macros are not fully compatible with interface of \verb,Reflection,,
...
\begin{verbatim}
  [ToXML ("Company.xml")] public module Company {
...
\begin{verbatim}
  ...
  def method_builder = c.DefineMethod ("ToXML", MethodAttributes.Public, 
                                       <[ type: void ]>, []);
  def list_flds = List.Map (flds, fun (x) {
