...
analyzed and the final result is compiled as a regular program. These operations 
...
We add some new ideas about syntax of meta-language, its much widened 
interoperability with compilation process and usage of new technologies to 
...
\begin{itemize}
  ...
  \item Using macros is transparent from user's point of view -- meta-program and 
    common function calls are indistinguishable, so user can use most complicated
...
and which generates. Although we allow only two level quotation (to avoid confusion
