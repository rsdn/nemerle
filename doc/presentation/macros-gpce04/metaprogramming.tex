...
\noindent
as a shortcut to call mentioned above.
...
example, allows to compute the size of the tuple described by the tup variable.
...
within the macro as real values (int, string, etc.),
% formatowanie? nie powinno byc \verb,int,, \verb,string,, ? 
but they are passed
...
the macro. Such design allows to distinguish easily which functions are run 
at the compile-time without any special annotations.

\subsection{Names binding in quotation}
A very important property of the meta-system is called \emph{hygiene} 
and related to the problem with names capture in Lisp macros, resolved 
...
To \_preserve\_ 
% niejasne - chodzi o avoid? - E.
names capture, all macro generated variables should be renamed 
...
In general, names in the generated code bind to definitions visible in 
...
to definitions visible at the construction of the quotation.
...
detected during the compilation of the code generated, we believe it is 
...
% "not matter... " niejasne, czy chodzi o cos w stylu: regardless of the context it comes from? E.
...
the namespace where the macro resides). In normal code it is done by \kw{using}
keyword, so we can omit the prefix of the full name, but this feature used in
the object code like
...
looked for in symbols imported at the quotation definition site.
...
as we put a special macro attribute \ref{Declarations} before that declaration.
Of course it must not be an external type and be available within the
compiled program as a source code.
% zdanie niejasne, czy chodzi o cos w rodzaju: Certainly, such a declaration must not be an external type 
% and has to be available within the compiled program as a source code? - E.
...
making code generation dependent on input programs types or enrichment
% nie wiem czy dobre slowo - improvement? enhancement? strengthening?
of the code analysis with additional information.
...
If \verb,if (x) 1 else "no", was written, the compiler would comply
that types of match cases do not fit each other. Such an error message could
be very confusing, because the programmer may not know, that his \verb,if,
statement is being transformed to the \verb,match, statement. Thus, we would
like to check few things 
% cos to potocznie brzmi, moze aspects? - E.
...
type. The body of the above \kw{if} macro is
...
us one level of this ``lifting'' up, when we are given the representation
of a program (syntax tree) and we must create a representation of such tree,
just as it would be (appears) the program. 
...
the real typing by compiler. It is dereferenced as if it was the result of recursive 
typing procedure. 
...
A key element of meta-programming is the execution of meta-programs during 
