...
\begin{verbatim}
  ...
  {
    def (columns, parameters, nquery) = parse_sql (query);
    <[ 
      def cmd = SqlCommand ($(nquery : string), conn);
...
\end{verbatim}
which returns modified query and lists of columns and parameters used 
in it. They are acoompanied with types, which must be obtained by accessing
