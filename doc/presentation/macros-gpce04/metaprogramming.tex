...
There are interesting differences between Template Haskell and Nemerle macros:
\begin{itemize}
...
\item Template Haskell macros are completely higher-order, like any other 
  ...
  compile-time. We decided to make macros first-class, so their usage looks like
  calling an ordinary function. We are still able to use higher-order functions
  in meta-language (functions operating on object code can be arbitrary), so only
  the top meta-function (prefixed with \verb,macro, is triggering compile-time 
  computations.
...
and splicing directly from it. Also entire paradigm of executing functions during compilation
and later type-checking their results come from Template Haskell.
