...
Of course, the framework is strongly \oo\ and primarily focused on traditional
...
messages hundred lines from the place of the real error. 
...
to determine usability of a programming language. 
%%% nie calkiem jasne - czy chodzi o "...that it is the quality of the design of the \oo\ system 
%%% that determines usability of a programming language. - E.?

While the existing \oo\ extensions 
...
%%% czy nie lepiej "but dynamic casting is available"? E.
...
how it is different from ML and \cs. The reader is assumed to have some
...
The top-level program structure resembles \cs. There are namespaces,
...
However, in the absence of generics support in the current Framework release
...
\begin{itemize}
  ...
  \item \kw{ensure}\ that need to come at the end of a block and can
    use \kw{value}\ keyword to refer to the value of a given block
\end{itemize}
...
\begin{itemize}
  ...
  \item \kw{guard} assertions that are checked after the update of any value 
...
%%% czy nie byloby lepiej "optimize the code for efficiency"? E.

Our meta-system is closely interleaved with the compilation process. 
It can perform guided by a user partial typing of the program AST. Compiler 
internal typing procedures are executed by the macro code in arbitrary order
...
%%% niejasne - pretty printing of the generated...? E.
  \end{itemize}
  ...
  \begin{itemize}
    ...
    \item support for Aspects-Oriented Programming by adding 
          cross-cutting ``concerns'' to the program in the algorithmic
...
\noindent is transformed into the following code:
...
In fact, in the 9th line of the output we see another extension, that is translated to:
...
Optimizations are performed on a different level. Semantic optimizations
...
transformation to argument assignment and goto. 
%%% formatowanie? - czy nie powinno byc \verb,goto,? E.
It brought a little speed improvement (over
...
and binary search automata. This is, however, in a very early stage yet.
