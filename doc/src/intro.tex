...
to functional languages are appealling because of their elegance, they do not fit 
...
Our meta-system is closely interweaved with compilation proccess. 
...
\begin{itemize}
  ...
  \begin{itemize}
    \item pretty printing generated or orginal code
  \end{itemize}
...
This is a sample implementation of a macro that selects 
an element from a tuple. Note that it is recursively using 
the \verb,for, macro. This code comes directly from 
the compiler implementation (consult the file 
\verb,extensions.n,).
  ...
  private class SelectFromTupleMacro implements IMacro {
    Run (args : list (PT.Parm)) : PT.Expr 
      ...
      match (get_params (args)) {
        | [ PT.E_literal (L_int (k)); 
            PT.E_literal (L_int (n)); 
            tupl ] =>
          mutable tup <- [];
          for (mutable i <- n; i > 0; i <- i - 1) {
            def ns = string_of_int (n - i + 1);
            tup <- PT.P_variable ("_N_tp" + ns) :: tup
          };
          <[ { def (.. $tup) = $tupl;
               $(PT.E_ref ("_N_tp" + 
                 string_of_int (k))) } ]>
        | _ => 
          Message.fatal_error 
            ("selectFromTuple macro expects 3 arguments")
      }
    }
  }
\end{Code}
...
the quotation construct in lines 14--16. A typed version of 
this macro could accept only two parameters -- the tuple and 
the index, the size of the tuple could be deducted after 
computing the tuple's type. The typed macros feature
is scheduled to be implemented soon.
...
call ellimination, inlining or matching automata generation) are performed
...
However it did not bring any improvments to the executction speed, it even slowed
things down by a factor of 15\%.

For tail calls to the current function, we have implemented simple
tranformation to argument assignment and goto. It brought little speed improvment (over
...
and binary search automatas. This is however in very early stage.
