...
languages to the \net\ do not fit is as well, as for example \cs \cite{CS}.

So we need it. But when we are at it, we can think about some other
...
members static) and variants (datatypes in SML, sum types in Caml). Let's
look at the famous example:
...
This macro requires SQL parser, and access to database we're working on,
