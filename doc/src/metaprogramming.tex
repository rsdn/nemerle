...
%
\title{Meta-programming in the New Functional \net\ Language \nem}
%
...
language \nem for \net\ platform. It intruduces support for set of 
...
and other inteoperability with compilation process, which can be fully
parametrized with any external data (like database, file or web page).

Our system provides good tool for Aspects Oriented Programming, which can 
operate on datatypes, traverse program's code and perform algorithmic addition 
of various concerns to its content.
\end{abstract}
...
In this process programs are becoming \emph{object programs}, which are data 
supplied to meta-programs. Then, they can be arbitrarily transformed or analyzed 
and final result is then compiled as regular program. These operations may be 
repeated or take place in stages. In latter case generated programs can generate
...
For example in our system, \verb,<[ 1 + f (2 * x) ]>, is annotation for syntax tree of 
expression \verb,1 + f (2 * x),. This idea is called 
...
by creating two sequences of expressions from lists returned by local 
functions \verb,create_parameters, and \verb,create_results,. 
Special syntax \verb,{.. $args }, is used to build sequence,  %$
which might have variable amount of arguments and we can't just write
\verb,{ e1; e2; ... },. 
...
Macro is the top-level function, which may have 
modifiers \verb,public,, or \verb,private, and resides in
standard \net/\nem namespace. Private macro may be accessed
only within file, where it's defined. It can have standard
name, which is considered in program as name of standard 
...
obvious since these values must be known in compile time).
...
into some arbitrary functions and run entire computation from within macro. 
...
Variables and constructor names from object language are always
dynamically scoped. They are bound where they appear in resulting
code, and not always where they appear in code. Programmer must be
careful about that, for example writing:

\begin{verbatim}
macro identity (e) { <[ def f (x) { x }; f($e) ]>
\end{verbatim} %$
might have surprising results when used by \verb,identity (f(1)),,
which would generate 
\begin{verbatim}
def f(x) { x }; f(f(1))
\enf{verbatim}

There are meta-systems like \Cite{MetaHaskell}, which use variable
names renaming in such cases, so \verb,f, from macro is different
from \verb,f, from code. This tactics is called ``hygienic'' macros
and is sometimes considered as better style of meta-programming
system. Our design decision to keep with dynamic scoping was conscious.
We find it much more flexible and natural in practice. 

Most important advantage is the possibility of easy composing code
from parts, which have common 

\subsection{Accesing compiler internals}
It's important for meta-functions to be able to use all benefits they have
from running in compile-time. They can retrieve informations from
compiler, use it's methods to access, analyze and change data stored
in its internal structures. 

\subsubsection{Retriving declarations}
...
like to check few thing during execution of our macro.
...
\begin{verbatim}
   ...
   match ( tcond.type.Equals (<[ ttype: bool ]>) ) {
     | true =>
       match ( te1.type.Unify (te2.type) ) {
         | Some =>
           <[ match ($(tcond : typed)) { 
                | true => $(te1 : typed) 
                | false => $(te2 : typed) } ]>
...
\end{verbatim}
%$
Note that typed expressions and again used in quotation, but with special 
...
it the list of \nem grammar elements (not typed syntax trees of object 
programs). 
...
Now splicing means just ``do not lift'', because we want to pass meta-language
expression object code. Of course it is only valid when this expression describes
(is type of) syntax tree. Operator \verb,.., inside quotation is translated as
syntax tree of list containing lifted expressions from provided list (which must
occur after \verb,..,).

One more thing to consider are the typed trees possibly stored in the result
of executing the macro. We need special element in not typed syntax tree, which
holds typed tree inside and which is treated in the special way during real
typing by compiler. It is dereferenced and took as it would be result of 
...
they are used. We must first type their bodies and also all classes and types, which
they are using.

To make these possible we have made few restrictions and set following order of
compilation of things related to macros:
\begin{itemize}
\item scan all type declarations, names of their fields and methods
\item find macros and type them using typing information collected in previous
      stage
\item add syntax extensions defined by macros to parser
\item mark all external (not local) types and methods they are using
\item recursively type and mark all declarations used directly or indirectly
      by macros
\item generate executable code for all those declarations
\item load it through \net System.Reflection, and instantiate one object for
      each macro
\item proceed with typing the rest of the program, executing corresponding
      \verb,Run, method for each occurrence of call to macro function.
\end{itemize}
