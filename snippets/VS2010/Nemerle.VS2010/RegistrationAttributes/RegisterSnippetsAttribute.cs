...
namespace Nemerle.VS2010.RegistrationAttributes
{
	/// <summary>
	/// This attribute registers code snippets for a package.  The attributes on
	/// a package do not control the behavior of the package, but they can be
	/// used by registration tools to register the proper information with 
	/// Visual Studio.
	/// </summary>
	...
	public sealed class RegisterSnippetsAttribute : RegistrationAttribute
	{
		...
		bool   _showRoots;
		short  _displayName;
		string _languageStringId;
		string _indexPath;
		string _forceCreateDirectories;
		string _paths;

		/// <summary>
		/// Creates a new RegisterSnippetsAttribute.
		/// </summary>
		public RegisterSnippetsAttribute(
			string languageGuid,
			bool   showRoots,
			short  displayName,
			string languageStringId,
			string indexPath,
			string forceCreateDirectories,
			string paths)
		{
		...
		/// </summary>
		/// <param name="context">
		/// Contains the location where the registration information should be 
		/// placed. It also contains other informations as the type being 
		/// registered and path information.
		/// </param>
		public override void Register(RegistrationContext context)
		{
			...
			using (Key childKey = context.CreateKey(LanguageName()))
			{
				...
				snippetIndexPath = context.EscapePath(snippetIndexPath);
				...
				snippetPaths = context.EscapePath(snippetPaths);

				// The following enables VS to look into a user directory for more
				// user-created snippets.
				//
				string myDocumentsPath = @";%MyDocs%\Code Snippets\" + NemerleConstants.LanguageName + @"\My Code Snippets\";
		...
		{
			if (context != null) context.RemoveKey(LanguageName());
		}
	}
}