...
{
    class NemerleNonCodeFileNode : CommonNonCodeFileNode
    {
        private object _designerContext;

        public NemerleNonCodeFileNode(CommonProjectNode root, ProjectElement e)
            : base(root, e)
        {
        }
        ...
        protected internal object DesignerContext
        {
            get
            {
                if (_designerContext == null)
                {
                    ...
                    var child = ProjectMgr.FindNodeByFullPath(filenameWithoutExt + ".n");

                    if (child != null)
                        XamlDesignerSupport.InitializeEventBindingProvider(_designerContext, (FileNode)child);
                }
