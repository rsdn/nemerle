...
namespace Nemerle.VisualStudio.Project
{
	...
	internal static class NemerleProjectMembers
	{
		...
		{
			var projectItems = project.BuildProject.AllEvaluatedItems;
			...
			foreach (var projectItem in projectItems)
			{
				if (folderMap != null &&
						folderMap.Count > 0 &&
					//TODO: отладит это дело. ѕон€ть можно ли было замен€ть projectItem.Name на projectItem.ItemType
						String.Equals(projectItem.ItemType, ProjectFileConstants.Folder, StringComparison.OrdinalIgnoreCase))
				{
					string relativePath = projectItem.EvaluatedInclude; //FinalItemSpec
		...
		/// </summary>
		/// <param name="projectItem">BuildItem to be checked.</param>
		/// <returns>Returns true if the buildItem is a file item, false otherwise.</returns>
		private static bool IsNemerleFileItem(ProjectItem projectItem)
		{
			ErrorHelper.ThrowIsNull(projectItem, "buildItem");

			bool isNemerleFileItem = false;             //buildItem.Name
			if (StringComparer.OrdinalIgnoreCase.Equals(projectItem.ItemType, ProjectFileConstants.Compile))
				isNemerleFileItem = true;
			else if (StringComparer.OrdinalIgnoreCase.Equals(projectItem.ItemType, ProjectFileConstants.EmbeddedResource))
				isNemerleFileItem = true;
			else if (StringComparer.OrdinalIgnoreCase.Equals(projectItem.ItemType, ProjectFileConstants.Content))
				isNemerleFileItem = true;
			else if (StringComparer.OrdinalIgnoreCase.Equals(projectItem.ItemType, "None"))
				isNemerleFileItem = true;
