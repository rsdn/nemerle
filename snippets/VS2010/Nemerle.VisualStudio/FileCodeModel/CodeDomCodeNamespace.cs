			...
			get
			{
				...
				foreach (CodeTypeDeclaration ctd in CodeObject.Types)
				{
					if (ctd.UserData[CodeKey] == null)
					{
						if (ctd.IsClass)
						{
							ctd.UserData[NemerleFileCodeModel.CodeKey] = CodeDomCodeClass.Create(DTE, this, ctd);
						}
						else if (ctd.IsInterface)
						{
							ctd.UserData[NemerleFileCodeModel.CodeKey] = new CodeDomCodeInterface(DTE, this, ctd);
						}
						else if (ctd.IsEnum)
						{
							ctd.UserData[NemerleFileCodeModel.CodeKey] = new CodeDomCodeEnum(DTE, this, ctd);
						}
						else if (ctd.IsStruct)
						{
							ctd.UserData[NemerleFileCodeModel.CodeKey] = new CodeDomCodeStruct(DTE, this, ctd);
						}
						else if (ctd is CodeTypeDelegate)
						{
							ctd.UserData[NemerleFileCodeModel.CodeKey] = new CodeDomCodeDelegate(DTE, this, (CodeTypeDelegate)ctd);
						}
					}
