...
namespace Microsoft.VisualStudio.Project
{
			...
			IVsCfgBrowseObject
	{
		...
		{
			CCITracing.TraceCall();
			string assembly = this.project.GetAssemblyName(this.ConfigKey);
		...
		internal static string TryGetActiveConfigurationAndPlatform(System.IServiceProvider serviceProvider, IVsHierarchy hierarchy)
		{
			string platform, configuration;

			if (Utilities.TryGetActiveConfigurationAndPlatform(serviceProvider, hierarchy, out configuration, out platform))
				return MakeConfigKey(configuration, platform);
