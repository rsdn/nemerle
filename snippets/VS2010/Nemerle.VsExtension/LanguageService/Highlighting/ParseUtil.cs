...
namespace Nemerle.VisualStudio.LanguageService
{
    public static class ParseUtil
    {
        ...
        public static bool TryGetTokenInfo(IVsTextLines vsTextLines, int line, int column, out TokenInfo tokenInfo)
        {
            tokenInfo = null;

            if (vsTextLines.GetPositionOfLineIndex(line, column, out int pos) != 0)
                return false;
            ...
            var source = VsSourceSnapshot.GetSourceSnapshot(textBuffer);
            var at = new Location(source, pos);
            return SearchToken(parseResult.Tokens, parseResult.Snapshot, at, false, ref tokenInfo);
        }

        private static bool SearchToken(Token token, ITextSnapshot snapshot, Location point, bool isQuotation, ref TokenInfo tokenInfo)
        {
            while (token != null)
            {
                if (point.StartPos < token.Location.StartPos)
                    break;
        ...
        private static bool SearchTokenTest(Token token, ITextSnapshot snapshot, Location point, bool isQuotation, ref TokenInfo tokenInfo)
        {
            if (!token.Location.Contains(point.StartPos))
                return false;
