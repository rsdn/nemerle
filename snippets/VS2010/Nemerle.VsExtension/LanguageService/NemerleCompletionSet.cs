        ...
        public override int GetInitialExtent(out int line, out int startIdx, out int endIdx)
        {
            ...
            if (ParseUtil.TryParse(textBuffer, out parseResult))
            {
                ...
                foreach (var spanInfo in spanInfos)
                    ...
                    {
                        var res = GetTokenExtent(caretPos, source, snapshot, spanInfo, out line, out startIdx, out endIdx);
                        return res;
                    }
            }
            ...
            switch (spanInfo.Type)
            {
                ...
                case SyntaxClassifier.SpanType.Operator:
                    if (!spanInfo.Span.IsEmpty && snapshot[spanInfo.Span.End - 1] == '.')
                        ...
                        goto default;
                case SyntaxClassifier.SpanType.Keyword:
                case SyntaxClassifier.SpanType.Identifier:
                    {
                        var loc  = Utils.ToLocation(new SnapshotSpan(snapshot, spanInfo.Span));
                        line     = loc.Line - 1;
                ...
                default:
                    {
                        var loc = Utils.ToLocation(new SnapshotSpan(snapshot, spanInfo.Span));
                        line = loc.Line - 1;
                        if (caretPos == loc.StartPos)
                            startIdx = endIdx = loc.Column - 1;
                        else
                            startIdx = endIdx = loc.EndColumn - 1;
                        return VSConstants.S_OK;
                    }
            }
        }
