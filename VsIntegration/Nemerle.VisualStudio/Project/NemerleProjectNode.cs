...
namespace Nemerle.VisualStudio.Project
{
  ...
  public class NemerleProjectNode : ProjectNode, IVsProjectSpecificEditorMap2
  {
    ...
    public override FileNode CreateFileNode(ProjectElement item)
    {
      ...
      //VladD2: Ётот код (добавлени€ CodeDomProvider-а) дико тормозил, так как дл€ каждого
      // файла проекта создавалс€ одтедльный экзепл€р провайдера. ѕровайдер за каким-то черотом
      // унаследован от ManagerClass и каждый раз инициализирует компил€тор (грузит все типы).
      //TODO: ѕереписать CodeDomProvider так чтобы он использовал систему типов проекта, не создавал
    ...
    protected override void OnHandleConfigurationRelatedGlobalProperties(object sender, ActiveConfigurationChangedEventArgs eventArgs)
    {
      ...
      _projectInfo.Engine.BeginReloadProject();
    }
