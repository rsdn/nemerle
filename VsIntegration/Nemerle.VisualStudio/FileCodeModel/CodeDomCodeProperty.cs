...
namespace Nemerle.VisualStudio.FileCodeModel
{
	...
	public class CodeDomCodeProperty : CodeDomCodeElement<CodeMemberProperty>, CodeProperty
	{
				...
				if (getter == null)
				{
					if (CodeObject.HasGet)
					{
						getter = new CodeDomCodeFunction(DTE,
							...
							CodeDomCodeTypeRef.FromCodeTypeReference(this.DTE, CodeObject.Type),
		...
		public CodeTypeRef Type
		{
			get
			{
				return CodeDomCodeTypeRef.FromCodeTypeReference(this.DTE, CodeObject.Type);
			}
			[SuppressMessage("Microsoft.Naming", "CA1725:ParameterNamesShouldMatchBaseDeclaration", MessageId = "0#")]
			set
			{
		...
		public string get_Prototype(int Flags)
		{
			...
			{
				CodeMemberProperty cmf = CodeObject as CodeMemberProperty;
