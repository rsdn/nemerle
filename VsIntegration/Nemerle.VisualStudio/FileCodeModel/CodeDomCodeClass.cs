...
namespace Nemerle.VisualStudio.FileCodeModel
{
	...
	public class CodeDomCodeClass : CodeDomCodeType<CodeTypeDeclaration>, CodeClass
	{
		...
		[SuppressMessage("Microsoft.Naming", "CA1704:IdentifiersShouldBeSpelledCorrectly", MessageId = "0#dte")]
		protected CodeDomCodeClass(DTE dte, CodeElement parent, CodeTypeDeclaration declaration)
		...
		public static CodeDomCodeClass Create(DTE dte, CodeElement parent, CodeTypeDeclaration declaration)
		{
			string topDecl = declaration.UserData["Nemerle.TopDeclaration"] as string;
			switch (topDecl)
			{
				case "Variant":
					return new CodeDomCodeVariant(dte, parent, declaration);

				case "VariantOption":
					return new CodeDomCodeVariant(dte, parent, declaration);

				default:
					return new CodeDomCodeClass(dte, parent, declaration);
			}
		}
