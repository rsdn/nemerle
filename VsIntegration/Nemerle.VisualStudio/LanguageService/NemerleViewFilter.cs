...
namespace Nemerle.VisualStudio.LanguageService
{
	partial class NemerleViewFilter : ViewFilter
	{
			...
			try
			{
				if (guidCmdGroup == Microsoft.VisualStudio.Project.VsMenus.guidStandardCommandSet2K)
				{
					if (_executingCommand == VsCommands2K.TAB)
					{
						...
						if (!this.Source.CompletionSet.IsDisplayed && this.Source.TryDoTableFormating(this, lineIndex + 1, colIndex + 1))
							return VSConstants.S_OK;
					}
				}
		...
		{
			//TODO: ¬ этом нужно разбиратьс€ и или выкидывать, или реализовывать по человечески
		...
		{
			//TODO: —оздать задачу дл€ данного вида рефакторинга и перенести эту реализацию в нее
